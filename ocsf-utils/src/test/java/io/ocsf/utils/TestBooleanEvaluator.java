...
public final class TestBooleanEvaluator extends TestCase
{
  public void testEvaluateFlat_BasicOperators()
  {
    ...
    node = BooleanExpression.parse("k3.x is_not null");
    Assert.assertTrue(BooleanEvaluator.evaluate(node, data));
...
//        // 01/04/10 01:32:27 UTC vs 01/04/10 01:32:27 PST
  ...
  public void testEvaluateFlatOperatorLikeAndNotLike()
  {
    ...
    node = BooleanExpression.parse("city not_like 'b'");
    Assert.assertFalse(BooleanEvaluator.evaluate(node, data));

    node = BooleanExpression.parse("city not_like 'culver'");
    Assert.assertTrue(BooleanEvaluator.evaluate(node, data));
    ...
    node = BooleanExpression.parse("name not_like 'ne  D'");
    Assert.assertTrue(BooleanEvaluator.evaluate(node, data));

    // "Piso" vs "PISO"
    node = BooleanExpression.parse("address like '8585 PISO 20-'");
    Assert.assertTrue(BooleanEvaluator.evaluate(node, data));
  }
  ...
  public void testEvaluateFlatOperatorMatchAndNotMatch()
  {
    ...
    node = BooleanExpression.parse("address match 'city,'");
    Assert.assertTrue(BooleanEvaluator.evaluate(node, data));
  }
  ...
  public void testEvaluateFlatOperatorsAndOr()
  {
    ...
    node = BooleanExpression.parse("name is_not null anD (contact like 'karla ' OR id = 42)");
    Assert.assertTrue(BooleanEvaluator.evaluate(node, data));
  }
  ...
  public void testEvaluateArray()
  {
    ...
    Assert.assertTrue(BooleanEvaluator.evaluate(
      BooleanExpression.parse("list contains Company = 'Acme'"), data));
