...
public final class ExpressionParser
{
  ...
  public static final Tree parse(final String exp) throws InvalidExpressionException
  {
  ...
  public final Tree parse() throws InvalidExpressionException
  {
  ...
  private final void term() throws InvalidExpressionException
  {
    if (token == Token.Not)
    {
      nextToken();
  ...
  private final void nextToken() throws InvalidExpressionException
  {
    token = readToken();
  }

  private final Token readToken() throws InvalidExpressionException
  {
  ...
  private final boolean isEqualSign()
  {
    if (buf[pos + 1] == '=')
    {
      ++pos;
  ...
  private final Token readSet() throws InvalidExpressionException
  {
  ...
  private final Object value() throws InvalidExpressionException
  {
    final int ch = buf[pos];
    switch (ch)
    {
  ...
  private static final boolean isDigit(final int ch)
  {
    return (ch >= '0' && ch <= '9') || ch == '.' || ch == '-' || ch == '+';
  }

  private static final boolean isExtendedDigit(final int ch)
  {
    return isDigit(ch) || ch == 'E' || ch == 'e' || ch == 'x' || ch == 'X';
  }

  private final Number number() throws InvalidExpressionException
  {
    int end = pos;
  ...
  private static final Number getNumber(final char[] buf, final int offset, final int len)
  {
  ...
  private static final Number getNumber(final String s)
  {
  ...
  private final char getEscapeChar() throws InvalidExpressionException
  {
    final int ch = buf[++pos];
    switch (ch)
    {
   ...
   */
  private final String getUnicode() throws InvalidExpressionException
  {
   ...
   * reached.
   */
  private static final int getSymbolSize(final char[] buf, final int pos, final int len)
  {
    int size = 0;
  ...
  private static final boolean isSymbolChar(final int ch)
  {
   ...
   */
  private final Object symbol() throws InvalidExpressionException
  {
    switch (buf[pos])
    {
      case 'T':
  ...
  private final Object symbol(final int len) throws InvalidExpressionException
  {
   ...
   */
  private final Token token() throws InvalidExpressionException
  {
  ...
  static final Token stringToToken(final String s)
  {
    switch (s.toLowerCase(Locale.US))
    {
      case "true":
   ...
   */
  private final String string() throws InvalidExpressionException
  {
    sb.setLength(0);
    try
    {
  ...
  private final Token readTimeValue() throws InvalidExpressionException
  {
   ...
   */
  private final InvalidExpressionException syntaxError(final String message)
  {
