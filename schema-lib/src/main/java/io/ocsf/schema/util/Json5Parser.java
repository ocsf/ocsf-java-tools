...
public final class Json5Parser
{
  ...
  public final <T> T to() throws ParserException
  {
  ...
  @SuppressWarnings("unchecked")
  public static final <T> T to(final String text) throws ParserException
  {
  ...
  @SuppressWarnings("unchecked")
  public static final <T> T to(final byte[] bytes, final int offset, final int length)
   ...
   * @throws ParserException If syntax error.
   */
  public static final Object parse(final String text) throws ParserException
  {
  ...
  public static final Object parse(final byte[] bytes) throws ParserException
  {
   ...
   * @throws ParserException If syntax error.
   */
  public final Object parse() throws ParserException
  {
    return value();
  }
   ...
   * @throws ParserException If syntax error.
   */
  private final Object value() throws ParserException
  {
    pos = skip(buf, pos, len);
   ...
   */
  private final String string() throws ParserException
  {
   ...
   * @throws ParserException If syntax error.
   */
  private final Map<String, Object> object() throws ParserException
  {
   ...
   * @throws ParserException If syntax error.
   */
  private final List<?> array(final int q) throws ParserException
  {
   ...
   */
  private final Object symbol() throws ParserException
  {
  ...
  private final String name(final int at)
  {
    int i = pos;
    while (buf[i] != ':' && buf[i] != '=')
    {
  ...
  private final String name(final int at, final int ch)
  {
    int i = pos;
    while (buf[i] != ch)
    {
  ...
  private static final int skipLine(final char[] buf, final int pos, final int len)
  {
    for (int i = pos; i < len; ++i)
    {
      if (buf[i] == '\n')
  ...
  private static final int skipLines(final char[] buf, final int pos, final int len)
  {
    for (int i = pos; i < len; ++i)
    {
      if (buf[i] == '*' && buf[i + 1] == '/')
  ...
  private final char getEscapeChar() throws ParserException
  {
    final int ch = next();
    switch (ch)
    {
   ...
   */
  private final ParserException syntaxError(final String message)
  {
    if (pos < buf.length)
    {
      return
